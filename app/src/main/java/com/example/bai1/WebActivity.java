...
public class WebActivity extends AppCompatActivity {
    ...
    public void xuly(String canh){
        int canhint = Integer.parseInt(canh);
        TinhToan toan = new TinhToan();
        toan.setCanh(canhint);
        int kqchuvi = toan.tinhChuVi();
        int kqdientich = toan.tinhDienTich();
        runOnUiThread(() -> webView.evaluateJavascript(
                "NhanKQ1('" + kqchuvi + "');", null
        ));
        runOnUiThread(() -> webView.evaluateJavascript(
                "NhanKQ2('" + kqdientich + "');", null
        ));
    }
}